...
contract Crowdsale is Whitelist {
    ...
    function buyTokens(address _beneficiary, uint256 _amount_tokens) public payable {
    ...
    fallback() external payable {
        buyTokens(msg.sender, _tokenAmount);
    }
    /*fallback() payable external {
        investisments[msg.sender] = investisments[msg.sender].add(msg.value);
        savedBalance = savedBalance.add(msg.value);
        // Il faut transférer l'équivalent en token 
        token.transfer(msg.sender, msg.value*rate);
